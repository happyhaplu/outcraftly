...
scp -i ~/.ssh/id_ed25519 scripts/ops/install-coolify.sh ubuntu@<YOUR_SERVER_IP>:/tmp/
ssh -i ~/.ssh/id_ed25519 ubuntu@<YOUR_SERVER_IP> "sudo bash /tmp/install-coolify.sh"
```
...
1. **Access Coolify Dashboard**
   ...
   - Or use tunnel: `ssh -i ~/.ssh/id_ed25519 -L 8000:localhost:8000 ubuntu@<YOUR_SERVER_IP>`
...
3. **Optional: Set Up Custom Domain for Coolify**
   - Point subdomain: `coolify.outcraftly.com` → `<YOUR_SERVER_IP>`
...
3. **DNS Already Points to Same IP**
   ...
   - Coolify uses same IP: `<YOUR_SERVER_IP>`
...
ssh -i ~/.ssh/id_ed25519 -L 8000:localhost:8000 ubuntu@<YOUR_SERVER_IP>
